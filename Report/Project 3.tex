...
\begin{figure}[H]
    ...
    \caption{The figure shows how the analytical solution decays exponentially over the domain x [0,1] and t [0,0.2]. There is an initial peaked temperature profile that gradually smooths and decreases in amplitude over time due to diffusion. }
    \label{fig:analytical}
\end{figure}

Figure \ref{fig:analytical} shows the behaviour of the analytical solution in the chosen time domain and over the whole spatial domain. The maximum value occurs at x = 0.5 at t = 0. The solution is symmetric around the midpoint, with a sinusoidal shape while flattening over time. 
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Comparison of Euler and PINN error heatmaps. Euler's error grows with time and remains largest in the spatial center. The PINN shows low error at small t, but error grows near t $\approx$ 0.03-0.05 before decaying again. Overall, PINN maintains lower maximum error.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.7\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Figures/Initial tests/long_time_0.8.png}
        \caption{Long time comparison of Euler, PINN and the analytical solution at t=0.8.}
        \label{fig:init_0.8}
    \end{subfigure}
    \caption{Long-time behaviour of Euler, PINN and the analytical solution. At t= 0.2, both numerical methods track the analytical curve well. At t=0.5, Euler slightly underestimates while PINN slightly overestimates the amplitude. At t=0.8 }
...
The initial tests confirm that both PINN and Euler reproduce the analytical solution of the diffusion equation with great accuracy for short times. 
